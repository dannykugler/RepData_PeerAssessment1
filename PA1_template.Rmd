...
```
The Mean number of steps is `r format(step.mm$mean.total.steps, 8.3)`
The Median number of steps is `r format(step.mm$median.total.steps, 8.3)`
...
The interval with the maximum number of steps is `r max.interval$interval` with `r format(max.interval$mean.steps, 6,2)` steps.
...
The number of missing values in the data set is `r format(nmiss, 5.0)`
...
The Mean number of steps is `r format(step.impute.mm$mean.total.steps, 8.3)` and
the Median number of steps is `r format(step.impute.mm$median.total.steps, 8.3)`

For the raw data, 
The Mean number of steps is `r format(step.mm$mean.total.steps, 8.3)` and
the Median number of steps is `r format(step.mm$median.total.steps, 8.3)`
